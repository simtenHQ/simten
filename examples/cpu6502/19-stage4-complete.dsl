// Stage 4 Complete: Stack & Subroutines
...
//
// This file integrates all Stage 4 components into a complete CPU.
...
// === Complete Control FSM with Stack & Subroutines ===
circuit Stage4Control {
  input reset: Bit
...
circuit Stage4CPU {
  ...
  impl {
    ...
    // ROM - Test program: subroutine call
    // $00: LDA #$00    (A9 00) - Clear A
    // $02: JSR $10     (20 10 00) - Call subroutine at $10
    // $05: STA $20     (85 20) - Store result to $20
    // $07: (end)
    //
    // Subroutine at $10:
    // $10: LDA #$42    (A9 42) - Load $42 into A
    // $12: RTS         (60) - Return
    node zero: Constant(value=0)
    node one: Constant(value=1)
    node two: Constant(value=2)
    ...
    node byte_3: Constant(value=16)   // 10 - addr low (subroutine at $10)
    node byte_4: Constant(value=0)    // 00 - addr high
    node byte_5: Constant(value=133)  // 85 - STA zp
    node byte_6: Constant(value=32)   // 20 - address $20
    ...
    node byte_16: Constant(value=169) // A9 - LDA #imm (at $10)
    node byte_17: Constant(value=66)  // 42 - operand ($42)
    node byte_18: Constant(value=96)  // 60 - RTS
    ...
    node mux19: Mux
    connect at_19.eq -> mux19.sel
    connect mux18.out -> mux19.in0
    connect byte_19.out -> mux19.in1

    // Instruction register
    node ir: Register
    connect clk -> ir.clk
    connect mux19.out -> ir.data

    // Operand register
    node operand_reg: Register
    connect clk -> operand_reg.clk
    connect mux19.out -> operand_reg.data

    // Address registers
    node addr_lo_reg: Register
    connect clk -> addr_lo_reg.clk
    connect mux19.out -> addr_lo_reg.data

    node addr_hi_reg: Register
    connect clk -> addr_hi_reg.clk
    connect mux19.out -> addr_hi_reg.data
