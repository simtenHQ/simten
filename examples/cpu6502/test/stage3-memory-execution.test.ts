...
describe('Stage 3 Memory (WORKING VERSION) - Execution Test', () => {
  ...
  it('should execute the working memory test program', () => {
    ...
    const debugResult = runSimulationTick(testCircuit, seqState);
    ...
    for (let cycle = 0; cycle < 25; cycle++) {
      ...
      const x = busToNumber(getPortValue(simResult.portValues, { nodeId: '', portName: 'reg_x' }));
    ...
    // Check final values
    const finalResult = runSimulationTick(testCircuit, seqState);
    const final_a = busToNumber(getPortValue(finalResult.portValues, { nodeId: '', portName: 'reg_a' }));
    const final_x = busToNumber(getPortValue(finalResult.portValues, { nodeId: '', portName: 'reg_x' }));
    const final_mem = busToNumber(getPortValue(finalResult.portValues, { nodeId: '', portName: 'mem_data' }));
