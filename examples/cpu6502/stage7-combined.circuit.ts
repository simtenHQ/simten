...
const ROM16K = component('ROM16K')
  ...
  .node('rom', ROM, { data: {"49152":56,"49153":120,"49154":8,"49155":24,"49156":88,"49157":40,"49158":169,"49159":15,"49160":41,"49161":240,"49162":9,"49163":240,"49164":200,"49165":200,"49166":202,"49167":234,"65532":0,"65533":192} })
  .connect(({ in: inp, out, c0, addr_hi_cmp, not_lt, addr_combine, rom }) => [
    inp.addr_hi.to(addr_hi_cmp.a, addr_combine.hi),
    c0.out.to(addr_hi_cmp.b),
    addr_hi_cmp.lt.to(not_lt.in),
    not_lt.out.to(out.responds),
    inp.addr_lo.to(addr_combine.lo),
    addr_combine.out.to(rom.addr),
    rom.data_out.to(out.data_out),
  ])
  .build()
...
const MemoryBus = component('MemoryBus')
  ...
  .connect(({ in: inp, out, ram, rom, not_rw, ram_we, open_bus, data_mux_ram, data_mux_rom, any_device, open_bus_we }) => [
    inp.addr_lo.to(ram.addr_lo, rom.addr_lo),
    inp.addr_hi.to(ram.addr_hi, rom.addr_hi),
    inp.data_in.to(ram.data_in),
    inp.rw.to(not_rw.in, open_bus_we.b),
    ram.responds.to(ram_we.a, data_mux_ram.sel, any_device.a),
    not_rw.out.to(ram_we.b),
    ram_we.out.to(ram.we),
    open_bus.q.to(data_mux_ram.in0),
    ram.data_out.to(data_mux_ram.in1),
    rom.responds.to(data_mux_rom.sel, any_device.b),
    data_mux_ram.out.to(data_mux_rom.in0),
...
const MemoryBusTest = component('MemoryBusTest')
  ...
  .connect(({ in: inp, out, mem_bus, addr_lo_in, addr_hi_in, data_in, rw_in, d_data_out }) => [
...
const CPU6502Test = component('CPU6502Test')
  ...
  .node('d_z', HexDisplay)
  .node('d_n', HexDisplay)
  .connect(({ in: inp, out, system, reset_input, d_pc_lo, d_pc_hi, d_instruction, d_a, d_x, d_y, d_sp, d_addr_lo, d_addr_hi, d_data, d_c, d_z, d_n }) => [
