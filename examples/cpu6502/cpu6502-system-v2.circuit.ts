...
const AddressDecode = component('AddressDecode')
  .in('addr_lo', bus(8))
  .in('addr_hi', bus(8))
  .in('rw', bit)
  .out('ram_cs', bit)
  ...
  .node('rom_range', Comparator)
  .node('rom_ge_c0', Not)
  .node('io_check', Comparator)
  ...
  .connect(({ in: inp, out, eight, ram_range, c0, f0, rom_range, rom_ge_c0, io_check, not_io_region, rom_select, zero, console_lo_check, console_select, not_rw, ram_write, console_write }) => [
    inp.addr_hi.to(ram_range.a, rom_range.a, io_check.a),
    eight.out.to(ram_range.b),
    ram_range.lt.to(out.ram_cs, ram_write.a),
    c0.out.to(rom_range.b),
    rom_range.lt.to(rom_ge_c0.in),
    f0.out.to(io_check.b),
    io_check.eq.to(not_io_region.in, console_select.a),
    rom_ge_c0.out.to(rom_select.a),
    not_io_region.out.to(rom_select.b),
    rom_select.out.to(out.rom_cs),
    inp.addr_lo.to(console_lo_check.a),
    zero.out.to(console_lo_check.b),
    console_lo_check.eq.to(console_select.b),
    console_select.out.to(out.console_cs, console_write.a),
    inp.rw.to(not_rw.in),
    not_rw.out.to(ram_write.b, console_write.b),
    ram_write.out.to(out.ram_we),
    console_write.out.to(out.console_we),
  ])
  .build()

const DataMux = component('DataMux')
  .in('ram_data', bus(8))
  .in('rom_data', bus(8))
  .in('ram_cs', bit)
  .in('rom_cs', bit)
  ...
  .connect(({ in: inp, out, open_bus, zero, mux_ram, mux_console, mux_rom, any_ram_rom, any_device, open_bus_we }) => [
    inp.ram_cs.to(mux_ram.sel, any_ram_rom.a),
    open_bus.q.to(mux_ram.in0),
    inp.ram_data.to(mux_ram.in1),
    inp.console_cs.to(mux_console.sel, any_device.b),
    mux_ram.out.to(mux_console.in0),
    zero.out.to(mux_console.in1),
    inp.rom_cs.to(mux_rom.sel, any_ram_rom.b),
    mux_console.out.to(mux_rom.in0),
    inp.rom_data.to(mux_rom.in1),
    mux_rom.out.to(out.data_out, open_bus.data),
    any_ram_rom.out.to(any_device.a),
    any_device.out.to(open_bus_we.a),
    inp.rw.to(open_bus_we.b),
...
const CPU6502Test = component('CPU6502Test')
  ...
  .node('reset_input', Input)
  .connect(({ in: inp, out, cpu, ram, rom, console, decode, data_mux, addr_combine, reset_input }) => [
    cpu.addr_lo.to(decode.addr_lo, addr_combine.lo),
    cpu.addr_hi.to(decode.addr_hi, addr_combine.hi),
    cpu.rw.to(decode.rw, data_mux.rw),
    ram.data_out.to(data_mux.ram_data),
    rom.data_out.to(data_mux.rom_data),
    decode.ram_cs.to(data_mux.ram_cs),
    decode.rom_cs.to(data_mux.rom_cs),
    decode.console_cs.to(data_mux.console_cs),
    addr_combine.out.to(rom.addr, ram.addr),
    cpu.data_out.to(ram.data_in, console.data),
    decode.ram_we.to(ram.we),
    decode.console_we.to(console.we),
    data_mux.data_out.to(cpu.data_in),
    reset_input.out.to(cpu.reset),
  ])
  .build()
