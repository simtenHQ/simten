// Stage 3 Complete: Absolute and Indexed Addressing Modes
// Adds 16-bit addressing and indexed modes (,X and ,Y)
// New instructions: LDA/STA with absolute and indexed modes

// === Simple Memory Controller ===
// Extended to handle 16-bit addresses
...
// === Complete Control FSM ===
// Handles immediate, zero-page, absolute, and indexed addressing
circuit CompleteControl {
  input reset: Bit
  ...
  impl {
    ...
    // Immediate: 2-byte, 2-cycle (sub0: fetch operand, sub1: execute)
    node is_imm: Or
    connect cmp_lda_imm.eq -> is_imm.a
    connect cmp_lda_imm.eq -> is_imm.b

    // Zero-page: 2-byte, 3-cycle (sub0: fetch addr, sub1: mem access, sub2: complete)
    node is_zp: Or
    connect cmp_lda_zp.eq -> is_zp.a
    connect cmp_sta_zp.eq -> is_zp.b

    // Absolute: 3-byte, 4-cycle (sub0: fetch lo, sub1: fetch hi, sub2: mem access, sub3: complete)
    node is_abs_temp: Or
    ...
    // 1-cycle: register ops (sub0: execute)
    node is_1cycle: Or
    ...
    // Done at sub3 for zero-page (one extra cycle for addr_lo_reg load)
    node done_zp: And
    connect exec_sub3.out -> done_zp.a
    connect is_zp.out -> done_zp.b

    // Done at sub4 for absolute (two extra cycles for addr_lo/hi_reg loads)
    node done_abs: And
    connect exec_sub4.out -> done_abs.a
    connect is_abs_final.out -> done_abs.b

    // Done at sub0 for 1-cycle
    node done_1cyc: And
    ...
    node exec_done: Or
    connect exec_done_temp2.out -> exec_done.a
    ...
    // Control signals
    // PC increment during FETCH and EXEC sub0 for multi-byte, EXEC sub1 for absolute
    ...
    // PC increment: after fetching each instruction byte
    // FETCH: increment after reading opcode
    // EXEC sub0: increment after reading first operand byte (for all multi-byte)
    // EXEC sub1: increment after reading second operand byte (for absolute only)
    ...
    // Operand loads during EXEC sub0 (for immediate and address values)
    ...
    // Address low loads during EXEC sub0 before PC increments (SUB1)
    ...
    // Address high loads during EXEC sub1 before PC increments (SUB2)
    ...
    // Memory read during EXEC sub1 (zp) or sub2 (abs) for loads
    node is_load: Or
    ...
    // Memory write during EXEC sub2 (zp) or sub3 (abs) for stores
    ...
    // Register A write during EXEC sub1 (imm), sub3 (zp), sub4 (abs)
    ...
    connect write_a_temp.out -> write_a_signal.a
    connect write_a_abs.out -> write_a_signal.b
...
circuit CompleteCPU {
  ...
  impl {
    ...
    // Program: A9 42 8D 10 00 AD 10 00 AA E8 (LDA #$42, STA $0010, LDA $0010, TAX, INX)
    // This tests absolute addressing!
    node byte_0: Constant(value=169)  // A9 - LDA #imm
    node byte_1: Constant(value=66)   // 42 - operand
    node byte_2: Constant(value=141)  // 8D - STA absolute
    node byte_3: Constant(value=16)   // 10 - address low
    node byte_4: Constant(value=0)    // 00 - address high (address = $0010)
    node byte_5: Constant(value=173)  // AD - LDA absolute
    node byte_6: Constant(value=16)   // 10 - address low
    node byte_7: Constant(value=0)    // 00 - address high
    node byte_8: Constant(value=170)  // AA - TAX
    node byte_9: Constant(value=232)  // E8 - INX
    node byte_10: Constant(value=0)   // padding
    ...
    node ir: Register
    connect clk -> ir.clk
    connect mux12.out -> ir.data

    // Operand register
    node operand_reg: Register
    connect clk -> operand_reg.clk
    connect mux12.out -> operand_reg.data

    // Address registers (low and high bytes)
    // Load directly from ROM mux (not from operand_reg)
    node addr_lo_reg: Register
    connect clk -> addr_lo_reg.clk
    connect mux12.out -> addr_lo_reg.data

    node addr_hi_reg: Register
    connect clk -> addr_hi_reg.clk
    connect mux12.out -> addr_hi_reg.data

    // For indexed addressing: add X to address
    // effective_addr = addr_lo + X (ignoring carries to high byte for simplicity)
    ...
    // Select address: indexed uses addr + X, others use addr_lo directly
    ...
    // Add X to address for indexed mode
    ...
    // Data source for A register (select based on addressing mode)
    // Immediate: operand_reg, Zero-page: memory, Absolute: memory
    ...
    node result_a: Mux
    connect control.is_lda_abs_x -> result_a.sel
    connect result_a_abs.out -> result_a.in0
    connect memory.data_out -> result_a.in1
