...
const FlagRegister = component('FlagRegister')
  ...
  .out('flag_c', bit)
  .out('flag_v', bit)
  .out('debug_update_z_received', bit)
  .out('debug_new_z_received', bit)
...
const Stage5Control = component('Stage5Control')
  .in('reset', bit)
...
const Stage5CPU = component('Stage5CPU')
  .in('reset', bit)
  .out('pc', bus(8))
  ...
  .out('flag_z', bit)
  .out('flag_c', bit)
  .out('debug_update_flags', bit)
  .out('debug_cmp_z_eq', bit)
  .out('debug_z_source', bit)
  .out('debug_is_cmp', bit)
  ...
  .node('byte_8', Constant, { value: 133 })
  .node('byte_9', Constant, { value: 32 })
  .node('byte_10', Constant, { value: 169 })
...
const Stage5Test = component('Stage5Test')
  .node('cpu', Stage5CPU)
