/**
 * Simulator v0.1 Tests (lib/simulator-v0.1.ts)
 *
 * Tests for the active production simulator used by Canvas and ClockControls.
...
describe('Simulator v0.1 (lib)', () => {
  ...
  describe('Topological Sort and Evaluation Order', () => {
    it('should detect cycles in the circuit', () => {
      ...
      expect(result.error).toBe('Cycle detected in circuit');
    });
  ...
  describe('Composite Component Evaluation', () => {
    it('should evaluate a composite HalfAdder component', () => {
      // First, register the HalfAdder as a user component
      ...
      library.registerUser(halfAdderCircuit);

      // Now create a circuit that uses the HalfAdder
      const circuit: Circuit = {
        id: 'test',
        name: 'HalfAdderTest',
        parameters: [],
      ...
      // 1 + 1 = 0 (sum) with 1 (carry)
      expect(result.portValues.get('ha1.sum')).toBe(false); // 1 XOR 1 = 0
      expect(result.portValues.get('ha1.carry')).toBe(true); // 1 AND 1 = 1
      expect(result.portValues.get('led1.in')).toBe(false);
      expect(result.portValues.get('led2.in')).toBe(true);
      expect(result.error).toBeUndefined();
    });
  });
