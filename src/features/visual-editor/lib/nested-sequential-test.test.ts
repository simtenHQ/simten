...
import { initializeSequentialState, runSimulationTick } from './simulator-v0.1';
...
describe('Nested Sequential Components', () => {
  ...
  it('should handle a simple register in a nested circuit', () => {
    ...
    const seqState = initializeSequentialState(wrapperCircuit);

    // Initial state - register should be 0
    expect(seqState.cycleCount).toBe(0);

    // Run one clock tick - register should latch the value 42
    const result1 = runSimulationTick(wrapperCircuit, seqState);
    expect(result1.error).toBeUndefined();
    expect(result1.sequentialState?.cycleCount).toBe(1);

    // CORRECTED: Check output using portValues, not node definition
    // Port definitions don't have .value - values are in simulation result's portValues Map
  ...
  it('should handle a counter with feedback in a nested circuit', () => {
    ...
    // Run 5 clock ticks
    let currentState = seqState;
    let lastResult;
    for (let i = 0; i < 5; i++) {
      lastResult = runSimulationTick(wrapperCircuit, currentState);
      expect(lastResult.error).toBeUndefined();
      currentState = lastResult.sequentialState!;
    }

    // CORRECTED: Check counter output using portValues from last result
    // Port definitions don't have .value - values are in simulation result's portValues Map
