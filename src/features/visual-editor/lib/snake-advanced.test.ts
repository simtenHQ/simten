...
import { compileToIR, type ComponentLibrary } from '../../dsl/compiler/ir-generator';
import {
...
describe('SnakeAdvanced - Pixel Address Storage (4 Phases)', () => {
  ...
  beforeEach(() => {
    ...
    const dslPath = path.join(process.cwd(), 'dsl-files/SnakeAdvanced.dsl');
  ...
  it('should initialize with 4-segment snake storing pixel addresses', () => {
    ...
    const ramMemory = seqState.currentState.get(ramId) as Map<number, number>;
  ...
  it('should calculate next head pixel address when moving right', () => {
    ...
    let seqState = initializeSequentialState(circuit);
    ...
    result = runCombinationalSimulation(circuit, seqState);
    expect(result.error).toBeUndefined();
  ...
  it('should simulate one complete movement cycle (4 phases)', () => {
    ...
    const ramId = findNodeId(circuit, 'ram');
    let ramMemory = seqState.currentState.get(ramId) as Map<number, number>;
    ...
    let result = runCombinationalSimulation(circuit, seqState);
    ...
    for (let step = 1; step <= 4; step++) {
      ...
      result = runSimulationTick(circuit, seqState);
      seqState = result.sequentialState!;
      ramMemory = seqState.currentState.get(ramId) as Map<number, number>;
