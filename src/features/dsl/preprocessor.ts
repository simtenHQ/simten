 ...
 * Note: This is a stub - actual implementation should use fs.readFileSync
 */
export function createNodeFileResolver(_basePath: string): FileResolver {
  // This would be implemented differently in Node.js vs browser
  // For now, return a stub that needs to be implemented by the caller
  return (_path: string, _fromPath?: string) => {
    // In Node.js, this would use fs.readFileSync
    // In browser, this would need a different mechanism
    console.warn('Node file resolver not implemented. Use custom resolver.');
    return null;
  };
}
