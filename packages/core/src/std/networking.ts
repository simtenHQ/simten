...
export const Eth_FrameInput = circuit('Eth_FrameInput', {
  ...
  onTick: ({ enable, reset, memory }) => {
    const mem = (memory as Map<number, number>) ?? new Map();
    const REG_READ_PTR = 0x10000, REG_FRAME_LEN = 0x10001, REG_INITIALIZED = 0x10002;
    const REG_OUT_TDATA = 0x10003, REG_OUT_TKEEP = 0x10004;
    const REG_OUT_TVALID = 0x10005, REG_OUT_TLAST = 0x10006, REG_OUT_OFFSET = 0x10007;

    if (reset) {
      const newMem = new Map(mem);
      newMem.set(REG_READ_PTR, 0); newMem.set(REG_INITIALIZED, 0);
      newMem.set(REG_OUT_TVALID, 0); newMem.set(REG_OUT_TLAST, 0);
    ...
    if (!(mem.get(REG_INITIALIZED) ?? 0)) {
      let maxAddr = -1;
      for (const addr of mem.keys()) { if (addr < 0x10000 && addr > maxAddr) maxAddr = addr; }
    ...
    if (readPtr >= frameLength || frameLength === 0) {
      newMem.set(REG_OUT_TDATA, 0); newMem.set(REG_OUT_TKEEP, 0);
      newMem.set(REG_OUT_TVALID, 0); newMem.set(REG_OUT_TLAST, 0);
    ...
    newMem.set(REG_OUT_TDATA, tdata); newMem.set(REG_OUT_TKEEP, tkeep);
    newMem.set(REG_OUT_TVALID, 1); newMem.set(REG_OUT_TLAST, isLast ? 1 : 0);
    newMem.set(REG_OUT_OFFSET, readPtr & 0xFFFF);
...
export const Eth_FrameParser = circuit('Eth_FrameParser', {
  ...
  eval: ({ memory }) => {
    ...
    return {
      dst_mac_hi: dstMacHi, dst_mac_lo: dstMacLo, dst_mac_valid: state >= 2 ? 1 : 0,
      src_mac_hi: srcMacHi, src_mac_lo: srcMacLo, src_mac_valid: state >= 4 ? 1 : 0,
      ethertype, ethertype_valid: state >= 6 ? 1 : 0,
      has_vlan: hasVlan ? 1 : 0, vlan_tci: vlanTci, vlan_valid: hasVlan && state >= 6 ? 1 : 0,
      payload_valid: state === 6 || state === 7 ? 1 : 0,
      frame_done: state === 7 ? 1 : 0,
      frame_length: byteCounter, parse_state: state,
    };
  },
  onTick: ({ tdata, tkeep, tvalid, tlast, memory }) => {
    ...
    const last = !!tlast;
    const R_STATE=0, R_DST_MAC_LO=1, R_DST_MAC_HI=2, R_SRC_MAC_LO=3, R_SRC_MAC_HI=4;
    const R_ETHERTYPE=5, R_VLAN_TCI=6, R_HAS_VLAN=7, R_BYTE_COUNTER=8;
    const IDLE=0, DST_MAC_HI_SRC=2, SRC_MAC=3, ETHERTYPE=4, VLAN=5, PAYLOAD=6, DONE=7;
    ...
    const b0=(td>>>24)&0xFF, b1=(td>>>16)&0xFF, b2=(td>>>8)&0xFF, b3=td&0xFF;
    switch (state) {
      case IDLE: newRegs.set(R_DST_MAC_LO, td); newRegs.set(R_STATE, DST_MAC_HI_SRC); break;
      case DST_MAC_HI_SRC:
        newRegs.set(R_DST_MAC_HI, (b0<<8)|b1); newRegs.set(R_SRC_MAC_LO, (b2<<8)|b3);
        newRegs.set(R_STATE, SRC_MAC); break;
      case SRC_MAC:
        newRegs.set(R_SRC_MAC_HI, regs.get(R_SRC_MAC_LO)??0 & 0xFFFF);
        newRegs.set(R_SRC_MAC_LO, td); newRegs.set(R_STATE, ETHERTYPE); break;
      case ETHERTYPE: {
        const etype = ((b0<<8)|b1)&0xFFFF;
        if (etype === 0x8100) {
          newRegs.set(R_VLAN_TCI, ((b2<<8)|b3)&0xFFFF);
          newRegs.set(R_HAS_VLAN, 1); newRegs.set(R_ETHERTYPE, etype); newRegs.set(R_STATE, VLAN);
        } else {
          newRegs.set(R_ETHERTYPE, etype); newRegs.set(R_STATE, last ? DONE : PAYLOAD);
        }
        break;
      }
      case VLAN:
        newRegs.set(R_ETHERTYPE, ((b0<<8)|b1)&0xFFFF); newRegs.set(R_STATE, last ? DONE : PAYLOAD); break;
      case PAYLOAD: if (last) newRegs.set(R_STATE, DONE); break;
    }
...
export const Eth_CRC32 = circuit('Eth_CRC32', {
  ...
  onTick: ({ data, data_valid, tkeep, tlast, reset, memory }) => {
    const regs = (memory as Map<number, number>) ?? new Map();
    if (reset) {
      const m = new Map(regs); m.set(0, 0xFFFFFFFF); m.set(1, 0); return { memory: m };
    }
    ...
    const m = new Map(regs); m.set(0, crc);
    if (tlast) m.set(1, 1);
    return { memory: m };
  },
...
export const MemBusMux = circuit('MemBusMux', {
  ...
  eval: ({ addr, write_data, mem_read, mem_write, funct3, read_data_0, read_data_1, read_data_2, read_data_3, read_data_4,
    ...
    const ranges = [
      { base: (((base0 as number) ?? 0x00010000) >>> 0), end: (((end0 as number) ?? 0x0001FFFF) >>> 0) },
      { base: (((base1 as number) ?? 0x80000000) >>> 0), end: (((end1 as number) ?? 0x80000FFF) >>> 0) },
      { base: (((base2 as number) ?? 0x80001000) >>> 0), end: (((end2 as number) ?? 0x80001FFF) >>> 0) },
      { base: (((base3 as number) ?? 0x80002000) >>> 0), end: (((end3 as number) ?? 0x80002FFF) >>> 0) },
      { base: (((base4 as number) ?? 0x00000000) >>> 0), end: (((end4 as number) ?? 0x0000FFFF) >>> 0) },
    ];
    let match = -1;
    for (let i = 0; i < 5; i++) { if (a >= ranges[i].base && a <= ranges[i].end) { match = i; break; } }
    const localAddr = match >= 0 ? ((a - ranges[match].base) >>> 0) : 0;
    const rdArr = [read_data_0, read_data_1, read_data_2, read_data_3, read_data_4].map(v => ((v as number) ?? 0) >>> 0);
    const readData = match >= 0 ? rdArr[match] : 0;
    const mr = !!mem_read, mw = !!mem_write;
    return {
      local_addr: localAddr, write_data_out: wd, funct3_out: f3, read_data: readData,
      p0_read: match===0&&mr?1:0, p0_write: match===0&&mw?1:0,
      p1_read: match===1&&mr?1:0, p1_write: match===1&&mw?1:0,
      p2_read: match===2&&mr?1:0, p2_write: match===2&&mw?1:0,
      p3_read: match===3&&mr?1:0, p3_write: match===3&&mw?1:0,
      p4_read: match===4&&mr?1:0, p4_write: match===4&&mw?1:0,
    };
  },
...
export const NIC_FIFO = circuit('NIC_FIFO', {
  ...
  eval: ({ tx_mem_read, rx_mem_read, tx_addr, rx_addr, memory }) => {
    ...
    let netTxData = 0, netTxValid = 0, netTxFrame = 0;
    if (draining && txDrainPtr < txWp) {
      netTxData = (state.get(0x0000 + txDrainPtr) ?? 0) >>> 0;
      netTxValid = 1;
      netTxFrame = (txDrainPtr + 1) >= txWp ? 1 : 0;
    }
    return { tx_read_data: txReadData, rx_read_data: rxReadData, net_tx_data: netTxData, net_tx_valid: netTxValid, net_tx_frame: netTxFrame };
  },
  onTick: ({ tx_addr, tx_write_data, tx_mem_write, rx_addr, rx_mem_write, net_rx_data, net_rx_valid, memory }) => {
    const state = (memory as Map<number, number>) ?? new Map();
    const newState = new Map(state);
    if (tx_mem_write) {
      const a = ((tx_addr as number) ?? 0) >>> 0;
      const d = ((tx_write_data as number) ?? 0) >>> 0;
      if (a === 0x0) { const wp = newState.get(0x2000)??0; newState.set(0x0000+wp, d); newState.set(0x2000, wp+1); }
      else if (a === 0xC) { newState.set(0x2002, 1); newState.set(0x2003, 0); newState.set(0x2004, 1); }
    }
    const wasDraining = state.get(0x2004) ?? 0;
    if (wasDraining) {
      const dp = newState.get(0x2003)??0, wp = newState.get(0x2000)??0;
      if (dp < wp) {
        newState.set(0x2003, dp+1);
        if ((dp+1) >= wp) { newState.set(0x2004,0); newState.set(0x2000,0); newState.set(0x2001,0); newState.set(0x2002,0); }
      }
    }
    if (net_rx_valid) {
      const d = ((net_rx_data as number) ?? 0) >>> 0;
      const wp = newState.get(0x2010)??0;
      newState.set(0x1000+wp, d); newState.set(0x2010, wp+1); newState.set(0x2012, (newState.get(0x2012)??0)+1);
    }
    if (rx_mem_write) {
      const a = ((rx_addr as number) ?? 0) >>> 0;
      if (a === 0x4) {
        const rp = newState.get(0x2011)??0, cnt = newState.get(0x2012)??0;
        if (cnt > 0) { newState.set(0x2011, rp+1); newState.set(0x2012, cnt-1); }
      }
    }
