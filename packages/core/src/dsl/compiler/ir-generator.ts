 ...
 *
 * Uses the core MutableComponentLibrary interface:
 *   resolveComponent(name) — look up a component
 *   addCircuit(circuit) — register a compiled circuit
 *   getAllComponentNames() — list all known components
 *
 * For backward compatibility, also accepts the legacy getCircuit/hasCircuit shape.
 */
...
/** Resolve a component from either the new or legacy interface */
function resolveFromLibrary(library: ComponentLibrary, name: string): Circuit | undefined {
  if (library.resolveComponent) return library.resolveComponent(name);
  if (library.getCircuit) return library.getCircuit(name);
  return undefined;
}
...
export class IRGenerator {
  ...
  private getAvailableComponentNames(): string[] {
    ...
    if (this.library.getAllPrimitiveNames) {
      return this.library.getAllPrimitiveNames();
    }
    return [];
  }
