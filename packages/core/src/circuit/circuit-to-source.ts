...
const STDLIB_NAMES: ReadonlySet<string> = new Set(
  STDLIB_CIRCUITS.map((c) => c.circuit.name),
);
...
function emitState(state: StateBlock[], circuitName: string): string {
  const entries = state.map((s) => {
    ...
    if (s.stateType.kind === "memory") {
      const mem = s.initialValue as { data: Map<number, number>; addressWidth: number; dataWidth: number };
...
function emitConnect(c: Circuit): string {
  ...
  const groups = new Map<string, { source: { nodeId: string; portName: string }; targets: { nodeId: string; portName: string }[] }>();
...
function portRef(
  p: { nodeId: string; portName: string },
  role: "source" | "target",
): string {
  if (p.nodeId === "") {
