...
import { compileDSL, type ComponentLibrary } from "@turing-incomplete/core/dsl";
 ...
 * Enhanced with initialMemory support for pre-loaded ROM data.
 */
export function useCircuitSimulator(
  ...
  // Track the initialMemory reference so we can detect changes
  const initialMemory = options?.initialMemory;

  // Compile DSL when it changes (expensive — ~5K lines for 6502)
  useEffect(() => {
    setReady(false);
    setError(null);
    setCycleCount(0);
  ...
  // This is separated from DSL compilation so that loading a new ROM
  // doesn't recompile the entire DSL — it just creates a fresh simulator.
  useEffect(() => {
  ...
  /** Get the underlying simulator engine (for imperative operations like runChecks) */
  const getSimulator = useCallback(() => simulatorRef.current, []);
