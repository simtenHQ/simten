...
test.describe('<circuit-embed> web component', () => {
  ...
  test('renders circuit nodes from source', async ({ page }) => {
    ...
    await expect(basicSection.locator('.react-flow__node').first()).toBeVisible({
      timeout: 10_000,
    });
  ...
  test("multiple instances don't crash", async ({ page }) => {
    ...
    await expect(page.locator('#test-basic .react-flow__node').first()).toBeVisible({
      timeout: 10_000,
    });
    await expect(page.locator('#test-isolation .react-flow__node').first()).toBeVisible({
      timeout: 10_000,
    });
