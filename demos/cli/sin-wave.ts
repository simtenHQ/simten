...
function render(values: number[]) {
  ...
  for (let r = 0; r < ROWS; r++) {
    ...
    const label = r === 0 ? '255' : r === ROWS - 1 ? '  0' : r % 5 === 0 ? String(logicalVal).padStart(3) : '   ';

    let row = '';
    for (let c = 0; c < cols; c++) {
      const u = upper[c], l = lower[c];
      if (u && l) row += FULL;
      else if (u)  row += UPPER;
      else if (l)  row += LOWER;
      else {
  ...
  const info = `\x1b[36m  addr: ${String(currentAddr ?? 0).padStart(3)}/255   value: ${String(history[history.length-1] ?? 0).padStart(3)}   cycle: ${String(history.length).padStart(5)}\x1b[0m`;
