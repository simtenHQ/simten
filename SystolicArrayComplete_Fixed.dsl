// ============================================================================
// Complete Systolic Array Implementation - FIXED VERSION
// Fixes: Done signal latch, capture timing, counter control
// ============================================================================
...
circuit SystolicArray2x2_Hardware {
  ...
  impl {
    ...
    // FIX: Add counter control - only count while computing, stop at cycle 6
    node is_cycle6: Comparator
    node not_done_counting: Not
    node counter_should_run: And
    ...
    // FIX: Capture at cycles 4 and 5 (adjusted for correct timing)
    // ========================================================================
    ...
    node is_cycle4: Comparator  // Capture row 0 results at cycle 4
    node is_cycle5: Comparator  // Capture row 1 results at cycle 5

    // ========================================================================
    // DONE SIGNAL GENERATION - FIX: Use OR feedback to latch
    // ========================================================================
    node is_cycle6_done: Comparator  // Signal done at cycle 6
    node done_reg: DFlipFlop
    node done_latch: Or  // Feedback OR to keep done=1 permanently
    ...
    // ========================================================================
    // CYCLE COUNTER: Increments while computing, stops at 6
    // ========================================================================
    connect counter.q -> counter_inc.in
    connect counter_inc.out -> counter_bits.in
    connect counter_bits.out -> counter.data

    // FIX: Stop counter at cycle 6
    connect counter.q -> is_cycle6.a
    connect const_6.out -> is_cycle6.b
    connect is_cycle6.eq -> not_done_counting.in
    connect start -> counter_should_run.a
    connect not_done_counting.out -> counter_should_run.b
    connect counter_should_run.out -> counter.we
    ...
    // ========================================================================
    // OUTPUT CAPTURE: FIX - Adjusted timing to cycle 4 and 5
    // ========================================================================
    // Capture row 0 results (c00, c01) at cycle 4
    ...
    // Capture row 1 results (c10, c11) at cycle 5
    ...
    // ========================================================================
    // DONE SIGNAL: FIX - Latch at cycle 6 using OR feedback
    // ========================================================================
    connect counter.q -> is_cycle6_done.a
    connect const_6.out -> is_cycle6_done.b

    // OR feedback: once done_reg.q becomes 1, it stays 1
    connect is_cycle6_done.eq -> done_latch.a
    connect done_reg.q -> done_latch.b
    connect done_latch.out -> done_reg.d
...
// ============================================================================
// PART 3: Demo Circuit - FIXED VERSION
// ============================================================================
...
// 5. Keep clocking - done LED will light and STAY on
...
circuit SystolicArrayDemo_Hardware {
  ...
  impl {
    ...
    // === Status Display ===
    node done_display: Led  // Lights up when computation complete and STAYS on
