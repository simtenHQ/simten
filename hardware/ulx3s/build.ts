#!/usr/bin/env tsx
/**
 * Build SnakeAdvanced → HDMI bitstream for ULX3S 85K.
 *
 * Usage (synth container must be running on port 8080):
 *   tsx hardware/ulx3s/build.ts           — build only, writes snake.bit
 *   tsx hardware/ulx3s/build.ts --flash   — build + flash via openFPGALoader
 *
 * Start container:
 *   docker run --rm -p 8080:8080 simten-synth-local
 */
...
console.log('Exporting SnakeAdvanced to Verilog...');
const { verilog: snakeVerilog, files } = exportVerilog(snakeCircuit, lib, {
  target: 'synthesis',
  topModuleName: 'SnakeAdvanced',
});
console.log(`  Verilog: ${(snakeVerilog.length / 1024).toFixed(1)} KB`);
...
const combinedVerilog = snakeVerilog + '\n\n' + wrapperVerilog;
...
const synthResp = await fetch(SYNTH_URL, {
  ...
  body: JSON.stringify({ verilog: combinedVerilog, files: files ?? {}, top: 'snake_top', target: 'ecp5' }),
}).then(r => r.json()) as { success: boolean; netlist?: string; stats?: object; log: string; error?: string };

if (!synthResp.success) {
  console.error('Synthesis failed:', synthResp.error);
  console.error(synthResp.log?.slice(-2000));
  process.exit(1);
}
...
}).then(r => r.json()) as { success: boolean; bitstream?: string; timing?: object; utilization?: object; log: string; error?: string };
...
// Step 3: Save bitstream
const bitData = Buffer.from(buildResp.bitstream!, 'base64');
const bitPath = resolve(__dirname, 'snake.bit');
