...
if (!example) { console.error('❌ no snake example'); process.exit(1); }
const result = executeCircuitCode(example.code);
if (result.error) { console.error(`❌ example failed to execute: ${result.error}`); process.exit(1); }
const playable = result.builtCircuits.find((c) => c.circuit.name === 'SnakePlayable');
if (!playable) { console.error('❌ SnakePlayable not among built circuits'); process.exit(1); }
...
function expectFbEqual(context: string): void {
  ...
  for (let a = 0; a < FB_SIZE; a++) {
    if (got[a] !== model.fb[a]) {
      console.error(`❌ ${context}: framebuffer mismatch at addr ${a}: ram=${got[a]} model=${model.fb[a]}`);
      console.error(`ram:\n${renderFb(got)}\nmodel:\n${renderFb(model.fb)}`);
      process.exit(1);
    }
  }
...
const KEYPRESSES = [
  null, 'right', 'right', 'up', // eats the food at (6,3) on the 4th tick
  'hold:30', null, 'left', 'left',
  'down', 'down', 'right', 'up',
] as const;
...
if (model.eaten < 1) { console.error(`❌ script expected at least one eat, got ${model.eaten}`); process.exit(1); }
sim.dispose();
console.log(`✅ playable harness works: ${KEYPRESSES.length} ticks driven by scan codes, fb matches model throughout (${model.eaten} food eaten, non-arrow key held direction)`);
