...
writeFileSync(
  spikeLd,
  readFileSync(resolve(HERE, 'link.ld'), 'utf8')
...
execFileSync(SPIKE, [
  '--isa=rv32i',
  '-m0x80000000:0x500000',
  `+signature=${spkSig}`,
  '+signature-granularity=4',
  spkElf,
]);
const ref = readFileSync(spkSig, 'utf8')
  .trim()
  .split('\n')
  .map((s) => s.trim());

const sym = (name: string): number => {
  const line = execFileSync(NM, [dutElf])
    .toString()
    .split('\n')
...
const injectSubForAdd = (imem: Uint8Array) => {
  for (let i = 0; i + 3 < imem.length; i += 4) {
    ...
    if ((w & 0x7f) === 0x33 && ((w >>> 12) & 7) === 0 && ((w >>> 25) & 0x7f) === 0) {
      ...
      imem[i] = bug & 0xff;
      imem[i + 1] = (bug >>> 8) & 0xff;
      imem[i + 2] = (bug >>> 16) & 0xff;
      imem[i + 3] = (bug >>> 24) & 0xff;
      flipped = true;
      console.log(
        `injected: add→sub at IMEM byte 0x${i.toString(16)} (0x${w.toString(16)} → 0x${bug.toString(16)})`,
      );
      return;
    }
  }
...
if (!flipped) {
  console.error('❌ no ADD found to corrupt — check failed to inject');
  process.exit(1);
}
...
if (diverged) {
  ...
  console.log(
    `✅ harness is valid: injected add→sub DIVERGES from Spike (first at word ${at}: dut ${dut[at]} vs spike ${ref[at]})`,
  );
  process.exit(0);
} else {
