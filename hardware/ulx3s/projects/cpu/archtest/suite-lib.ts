...
const sym = (elf: string, name: string): number => {
  const line = execFileSync(NM, [elf])
    .toString()
    .split('\n')
...
const trapOps = (elf: string): number =>
  (
    execFileSync(OBJDUMP, ['-d', elf], BIGBUF)
      .toString()
      .match(/\t(csrr?[wsci]?|ecall|ebreak|mret|sret|wfi)\b/g) ?? []
  ).length;
...
export function runOneTest(t: string): Result {
  ...
  try {
    ...
    const dut = runDut(
      dutElf,
      sym(dutElf, 'begin_signature'),
      sym(dutElf, 'end_signature'),
      sym(dutElf, 'tohost'),
    );
    ...
    return {
      ...
      trapFree: false,
      skipped: false,
      note: `ERROR: ${(e as Error).message.split('\n')[0]}`,
    };
  }
}
