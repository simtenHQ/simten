...
function encode_add(rd: number, rs1: number, rs2: number): number {
  return (
    (0 << 25) | ((rs2 & 0x1f) << 20) | ((rs1 & 0x1f) << 15) | (0 << 12) | ((rd & 0x1f) << 7) | 0x33
  );
}
...
function encode_and(rd: number, rs1: number, rs2: number): number {
  return (
    (0 << 25) | ((rs2 & 0x1f) << 20) | ((rs1 & 0x1f) << 15) | (7 << 12) | ((rd & 0x1f) << 7) | 0x33
  );
}

function encode_or(rd: number, rs1: number, rs2: number): number {
  return (
    (0 << 25) | ((rs2 & 0x1f) << 20) | ((rs1 & 0x1f) << 15) | (6 << 12) | ((rd & 0x1f) << 7) | 0x33
  );
}

function encode_xor(rd: number, rs1: number, rs2: number): number {
  return (
    (0 << 25) | ((rs2 & 0x1f) << 20) | ((rs1 & 0x1f) << 15) | (4 << 12) | ((rd & 0x1f) << 7) | 0x33
  );
}

function encode_sll(rd: number, rs1: number, rs2: number): number {
  return (
    (0 << 25) | ((rs2 & 0x1f) << 20) | ((rs1 & 0x1f) << 15) | (1 << 12) | ((rd & 0x1f) << 7) | 0x33
  );
}

function encode_srl(rd: number, rs1: number, rs2: number): number {
  return (
    (0 << 25) | ((rs2 & 0x1f) << 20) | ((rs1 & 0x1f) << 15) | (5 << 12) | ((rd & 0x1f) << 7) | 0x33
  );
}
...
function encode_slt(rd: number, rs1: number, rs2: number): number {
  return (
    (0 << 25) | ((rs2 & 0x1f) << 20) | ((rs1 & 0x1f) << 15) | (2 << 12) | ((rd & 0x1f) << 7) | 0x33
  );
}

function encode_sltu(rd: number, rs1: number, rs2: number): number {
  return (
    (0 << 25) | ((rs2 & 0x1f) << 20) | ((rs1 & 0x1f) << 15) | (3 << 12) | ((rd & 0x1f) << 7) | 0x33
  );
}
...
test(
  'R-Type',
  'SLL uses only low 5 bits of rs2',
  fw(
    SET_UART_A5,
    encode_addi(a0, x0, 1),
...
test(
  'Load',
  'LH sign-extends negative halfword',
  fw(
    SET_UART_A5,
    encode_lui(t0, 0x10),
...
test(
  'Branch',
  'BEQ not-taken: next instr executes',
  fw(
    SET_UART_A5,
    encode_addi(a0, x0, 5),
    encode_addi(a1, x0, 6),
...
test(
  'Branch',
  'BLT signed: large positive not < -1',
  fw(
    SET_UART_A5,
    encode_lui(a0, 0x7ffff),
...
test(
  'Branch',
  'Forward branch max range (skip large block)',
  fw(
    SET_UART_A5,
...
test(
  'Hazard',
  'WB bypass LUI to branch',
  fw(
    SET_UART_A5,
    encode_lui(a0, 1), // a0 = 0x1000
    NOP,
    NOP, // 2 nops: LUI reaches WB
...
test(
  'Hazard',
  'JAL flush: instruction after JAL skipped',
  fw(
    SET_UART_A5,
...
test(
  'System',
  'FENCE does not corrupt pipeline',
  fw(
    SET_UART_A5,
    encode_lui(t0, 0x10),
    encode_addi(a0, x0, 0x41),
...
test(
  'System',
  'ECALL does not corrupt state before it',
  fw(
    SET_UART_A5,
    encode_lui(t0, 0x10),
    encode_addi(a0, x0, P),
  ...
  [P],
  100,
); // short cycle limit: if ECALL loops forever this fails fast

// EBREAK: same contract as ECALL
test(
  'System',
  'EBREAK does not corrupt state before it',
  fw(
    SET_UART_A5,
    encode_lui(t0, 0x10),
    encode_addi(a0, x0, P),
...
test(
  'Misalign',
  'Misaligned LW does not corrupt pipeline',
  fw(
    SET_UART_A5,
    encode_lui(t0, 0x10),
    encode_addi(a0, x0, P),
...
test(
  'Misalign',
  'Misaligned LH does not corrupt pipeline',
  fw(
    SET_UART_A5,
    encode_lui(t0, 0x10),
    encode_addi(a0, x0, P),
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  for (const t of selected) {
    ...
    const ok = actual.length >= t.expected.length && t.expected.every((b, i) => actual[i] === b);
