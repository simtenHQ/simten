circuit MacRxParser {
  ...
  impl {
    ...
    // State constants
    node STATE_IDLE: Input           // 0
    node STATE_PREAMBLE_SYNC: Input  // 1
    node STATE_WAIT_SFD: Input       // 2
    node STATE_IN_FRAME: Input       // 3

    // Preamble/SFD constants
    node PREAMBLE_BYTE: Input   // 0x55 (85)
    node SFD_BYTE: Input        // 0xD5 (213)
    node SEVEN: Input           // 7
    node EIGHT: Input           // 8
    ...
    connect SEVEN.out -> preamble_is_seven.b
    ...
    // PREAMBLE_SYNC → WAIT_SFD (when count == 7 and byte == 0x55)
    node preamble_to_wait: And
    connect isPREAMBLE_SYNC.eq -> preamble_to_wait.a
    connect preamble_is_seven.eq -> preamble_to_wait.b

    node preamble_to_wait_valid: And
    connect preamble_to_wait.out -> preamble_to_wait_valid.a
    connect valid -> preamble_to_wait_valid.b

    node preamble_to_wait_match: And
    connect preamble_to_wait_valid.out -> preamble_to_wait_match.a
    connect isPreambleByte.eq -> preamble_to_wait_match.b

    // PREAMBLE_SYNC → IDLE (preamble broken)
    ...
    connect not_preamble_byte.out -> preamble_broken.b
    ...
    // Priority: frame_done → sfd_transition → preamble_to_wait → idle_transition → stay in current state

    // Mux level 4: frame_done? IDLE : (lower priority)
    node next_state_m4: Mux
    connect STATE_PREAMBLE_SYNC.out -> next_state_m4.in0  // Will be replaced
    connect STATE_IDLE.out -> next_state_m4.in1
    connect frame_done.out -> next_state_m4.sel

    // Mux level 3: sfd_transition? IN_FRAME : next_state_m4
    node next_state_m3: Mux
    connect next_state_m4.out -> next_state_m3.in0
    connect STATE_IN_FRAME.out -> next_state_m3.in1
    connect sfd_transition.out -> next_state_m3.sel
    ...
    node fsm_state_we: Switch  // Always write
...
circuit IngressController {
  ...
  impl {
    ...
    // State constants
    node STATE_IDLE: Input           // 0
    node STATE_RECEIVING: Input      // 1
    node STATE_BUFFERED: Input       // 2

    // Numeric constants
    node ZERO: Input
    node ONE: Input
    node FOUR: Input                 // Max packets
    node SEVEN: Input                // Bytes per packet - 1
    node EIGHT: Input                // Bytes per packet
    ...
    node MINUS_ONE: Input  // 255 (for subtraction)
...
circuit PacketForwarder2Port {
  ...
  impl {
    ...
    // State constants
    node STATE_IDLE: Input           // 0
    node STATE_READ_HEADER: Input    // 1
    node STATE_WAIT_HEADER: Input    // 2
    node STATE_ROUTE: Input          // 3
    node STATE_COPY_PAYLOAD: Input   // 4
    node STATE_DONE: Input           // 5
    ...
    node DA_MASK: Input          // 0xF0 (upper 4 bits)
    node DA_SHIFT: Input         // 4 (shift right by 4)
    ...
    node ingress_port_reg_we: Switch
    ...
    node output_port_reg_we: Switch
    ...
    node THREE: Input  // Shift by 3 = multiply by 8
