...
const DISASM: DisasmRow[] = [
  { kind: 'label', text: '<_start>:' },
  {
    ...
    addr: '00000000',
    text: 'auipc sp,0x20',
    note: 'Compute the stack pointer from the program counter',
  },
  {
    kind: 'instr',
    addr: '00000004',
    text: 'mv sp,sp',
    note: 'Finish setting up the stack pointer',
  },
...
function Spinner() {
  return (
    <span className="inline-block h-3.5 w-3.5 shrink-0 animate-spin rounded-full border-2 border-muted-foreground/30 border-t-foreground" />
  );
}
...
export function RV32IDebuggerPreview() {
  ...
  useEffect(() => {
    ...
    const start = () => {
      if (id) return;
      if (reduce) {
        setTick(COMPILE_TICKS);
        return;
      } // park on a static frame
      setTick(0);
