...
const HANDLERS: Record<string, Handler> = {
  ...
  lhu:  (_, o) => `Load halfword unsigned: read 16 bits (zero-extended) from [${o[1]}] into ${reg(o[0])}.`,
  lb:   (_, o) => `Load byte: read 8 bits (sign-extended) from [${o[1]}] into ${reg(o[0])}.`,
  lbu:  (_, o) => `Load byte unsigned: read 8 bits (zero-extended) from [${o[1]}] into ${reg(o[0])}.`,
  ...
  mulh: (_, o) => `Multiply high: ${reg(o[0])} = upper 32 bits of ${reg(o[1])} × ${reg(o[2])} (signed).`,
  ...
  srli: (_, o) => `Shift right logical immediate: ${reg(o[0])} = ${reg(o[1])} >> ${o[2]} (zero-fill).`,
  sra:  (_, o) => `Shift right arithmetic: ${reg(o[0])} = ${reg(o[1])} >> ${reg(o[2])} (sign-extend).`,
  srai: (_, o) => `Shift right arithmetic immediate: ${reg(o[0])} = ${reg(o[1])} >> ${o[2]} (sign-extend).`,

  // --- Comparisons ---
  slt:  (_, o) => `Set if less than: ${reg(o[0])} = 1 if ${reg(o[1])} < ${reg(o[2])} (signed), else 0.`,
  slti: (_, o) => `Set if less than immediate: ${reg(o[0])} = 1 if ${reg(o[1])} < ${o[2]} (signed), else 0.`,
  sltu: (_, o) => `Set if less than unsigned: ${reg(o[0])} = 1 if ${reg(o[1])} < ${reg(o[2])} (unsigned), else 0.`,
  sltiu:(_, o) => `Set if less than immediate unsigned: ${reg(o[0])} = 1 if ${reg(o[1])} < ${o[2]} (unsigned), else 0.`,
  ...
  blt:  (_, o) => `Branch if less than: jump to ${addr(o[2])} if ${reg(o[0])} < ${reg(o[1])} (signed).`,
  bge:  (_, o) => `Branch if greater or equal: jump to ${addr(o[2])} if ${reg(o[0])} ≥ ${reg(o[1])} (signed).`,
  bltu: (_, o) => `Branch if less than unsigned: jump to ${addr(o[2])} if ${reg(o[0])} < ${reg(o[1])} (unsigned).`,
  bgeu: (_, o) => `Branch if greater or equal unsigned: jump to ${addr(o[2])} if ${reg(o[0])} ≥ ${reg(o[1])} (unsigned).`,
  ...
  auipc: (_, o) => `Add upper immediate to PC: ${reg(o[0])} = PC + (${o[1]} << 12). Used to compute addresses relative to the current instruction.`,
};

function findPrefixHandler(mnemonic: string): Handler | null {
  // Handle fence.i, etc.
  if (mnemonic.startsWith("fence")) return () => "Memory fence — enforces ordering of memory operations.";
  return null;
}
