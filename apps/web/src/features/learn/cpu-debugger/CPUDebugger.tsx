...
const PIPELINE_DESCRIPTIONS: Record<
  keyof PipelineStages,
  { label: string; title: string; desc: string }
> = {
  IF: {
    label: 'Fetch',
    title: 'Instruction Fetch',
    desc: 'Read the next instruction from memory at the current program counter (PC).',
  },
  ID: {
    label: 'Decode',
    title: 'Instruction Decode',
    desc: 'Decode the instruction, read source registers from the register file, and generate control signals.',
  },
  EX: {
    label: 'Compute',
    title: 'Execute',
  ...
  MEM: {
    label: 'Memory',
    title: 'Memory Access',
    desc: 'Load or store data in RAM. For non-memory instructions this stage passes values through unchanged.',
  },
  WB: {
    label: 'Save',
    title: 'Write Back',
    desc: 'Write the result back to the destination register in the register file.',
  },
};
...
function describeCurrentCycle(
  ...
  if (cycleCount === 0)
    return 'Press Step to advance one clock cycle, or Run to execute continuously.';
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <div
          className={`flex flex-col rounded-lg border px-2 py-1.5 cursor-help min-w-0 flex-1 ${color}`}
        >
...
function DisasmPane({ lines, stages }: { lines: DisasmLine[]; stages: PipelineStages }) {
  const activePCs = new Set(Object.values(stages).filter((v): v is number => v != null));
    ...
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-x-3 gap-y-0.5 font-mono text-xs">
      {ordered.map((i) => {
        ...
        return (
          <div
            key={i}
            className={`flex items-center gap-1.5 py-0.5 rounded px-1 -mx-1 transition-colors ${isChanged ? 'bg-green-500/10' : ''}`}
          >
            <span
              className={`w-7 shrink-0 text-[11px] ${isChanged ? 'text-green-400' : 'text-gray-500'}`}
            >
              {ABI_NAMES[i]}
            </span>
            <span
              className={`tabular-nums truncate ${isChanged ? 'text-green-300' : nonzero ? 'text-gray-200' : 'text-gray-700'}`}
            >
              {hex(val)}
            </span>
          </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-1 flex-col overflow-hidden border-r border-gray-800">
          ...
          <div className="shrink-0 border-b border-gray-800 px-3 py-2">
            ...
            <TooltipProvider delayDuration={300}>
              <div className="flex gap-1.5 overflow-x-auto">
                {(['IF', 'ID', 'EX', 'MEM', 'WB'] as (keyof PipelineStages)[]).map((s) => {
                  ...
                  const rawInstr =
                    pc != null
                      ? (disasmLines.find((l) => l.address === pc && l.instruction)?.instruction ??
                        null)
                      : null;
            ...
            {narrative && (
              <p className="mt-1.5 text-[11px] text-gray-400 leading-relaxed">{narrative}</p>
            )}
          </div>
