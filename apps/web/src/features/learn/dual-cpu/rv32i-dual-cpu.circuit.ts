...
// See `cpu-debugger/rv32i-cpu.circuit.ts` for docs on the debug_addr/debug_value
// scan-port pattern. Each CPU here exposes its own pair, wired up to the
// RV32I_DualCPU wrapper below as debug_addr0/debug_value0 and _1.
...
export const RV32I_DualCPU = circuit('RV32I_DualCPU', {
  inputs: { debug_addr0: bus(5), debug_addr1: bus(5) },
  outputs: { cpu0_pc: bus(32), cpu1_pc: bus(32), debug_value0: bus(32), debug_value1: bus(32) },
  nodes: { cpu0: RV32I_CPU, cpu1: RV32I_CPU },
  connect: ({ inputs, outputs, nodes: { cpu0, cpu1 } }) => [
    ...
    cpu0.pc_out.to(outputs.cpu0_pc),
    cpu1.pc_out.to(outputs.cpu1_pc),
    inputs.debug_addr0.to(cpu0.debug_addr),
    inputs.debug_addr1.to(cpu1.debug_addr),
    cpu0.debug_value.to(outputs.debug_value0),
    cpu1.debug_value.to(outputs.debug_value1),
  ],
})
