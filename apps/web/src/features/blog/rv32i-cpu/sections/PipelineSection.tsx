    ...
    <section className="py-12">
      ...
      <div className="prose-invert space-y-6">
        ...
        <p className="text-gray-600 dark:text-gray-300 leading-relaxed">
          ...
          <strong className="text-gray-900 dark:text-white">stages</strong>, separated by registers.
          Each stage does one piece of the work, then passes its result to the next stage on the
          clock edge. Like an assembly line: while one instruction is being executed, the next is
          being decoded, and the one after that is being fetched.
        </p>
        <p className="text-gray-600 dark:text-gray-300 leading-relaxed">
          The result:{' '}
          <strong className="text-gray-900 dark:text-white">
            5 instructions in flight at once
          </strong>
          . The clock can tick as fast as the slowest <em>stage</em>, not the slowest instruction.
          In practice, this roughly quintuples throughput.
        </p>
      </div>
      ...
      <div className="mt-8 space-y-4">
        {STAGES.map((stage) => (
          <div key={stage.abbr} className={`rounded-xl border p-5 ${STAGE_COLORS[stage.color]}`}>
      ...
      <div className="mt-10">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">
          ...
          sources: <strong className="text-gray-900 dark:text-gray-200">PC + 4</strong> for
          sequential execution,{' '}
          <strong className="text-gray-900 dark:text-gray-200">PC + immediate</strong> for branches
          and JAL, or{' '}
          <strong className="text-gray-900 dark:text-gray-200">register + immediate</strong> for
          JALR (indirect jumps). When a branch is taken, the pipeline flushes the wrongly-fetched
          instructions and redirects to the target address.
        </p>
        <CircuitEmbed
        ...
        <div className="mt-6">
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">
            Here&rsquo;s the full version with the mux. Toggle{' '}
            <strong className="text-gray-900 dark:text-gray-200">branch</strong> to redirect the PC
            to address 0x100, <strong className="text-gray-900 dark:text-gray-200">jump</strong> to
            redirect to 0x400, or{' '}
            <strong className="text-gray-900 dark:text-gray-200">stall</strong> to freeze it
            entirely (like a load-use hazard). Turn them off and the PC resumes incrementing by 4
