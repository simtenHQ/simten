...
const advance = (s: Sim, ticks: number) => { for (let i = 0; i < ticks; i++) s.tick(); };
...
verify.exhaustive("ball moves and stays on the 32x16 field", [1], () => {
  ...
  try {
    ...
    for (let k = 0; k < 12; k++) {
      advance(s, 8);
      if (s.get("is_filling")) { settle(s); continue; }          // skip a death's redraw
...
verify.exhaustive("wall is drawn; ball rebounds off the face without tunnelling", [1], () => {
  ...
  try {
    ...
    for (let k = 0; k < 30; k++) {
      advance(s, 8);
      if (s.get("is_filling")) { settle(s); missing = 0; continue; }
      if (ballCells(s).length === 0) missing++; else missing = 0;
