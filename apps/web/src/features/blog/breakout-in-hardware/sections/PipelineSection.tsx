      ...
      <div className="prose-invert space-y-6">
        ...
        <div className="my-6 grid grid-cols-2 sm:grid-cols-5 gap-2">
          {[
            ...
            >
              <div className="text-[10px] font-mono text-gray-500 dark:text-gray-500">Phase {phase}</div>
              <div className={`text-xs font-semibold ${
                label === "Clear" ? "text-red-400" :
                label === "Draw" ? "text-green-400" :
                label === "Read" ? "text-blue-400" : "text-amber-400"
              }`}>{label}</div>
              <div className="text-[10px] text-gray-500 dark:text-gray-500 mt-0.5">{desc}</div>
            </div>
          ))}
        </div>
        <p className="text-gray-600 dark:text-gray-300 leading-relaxed">
