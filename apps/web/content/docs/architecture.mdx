  ...
  return (
    <div className="flex gap-4 mb-1">
      <div className="flex flex-col items-center">
        ...
        {number < 6 && <div className="w-px flex-1 bg-border mt-1" />}
      </div>
      <div className="pb-6">
        <h3 className="text-base font-semibold text-foreground mt-1">{title}</h3>
        <p className="text-sm text-muted-foreground mt-0.5">{subtitle}</p>
        {children && <div className="text-sm text-muted-foreground mt-2">{children}</div>}
      </div>
    </div>
  );
}
...
export function PhaseStep({ phase, title, children }) {
  const colors = {
    1: 'bg-blue-100 border-blue-300 dark:bg-blue-900/50 dark:border-blue-700/50',
    2: 'bg-amber-100 border-amber-300 dark:bg-amber-900/50 dark:border-amber-700/50',
    3: 'bg-purple-100 border-purple-300 dark:bg-purple-900/50 dark:border-purple-700/50',
    4: 'bg-green-100 border-green-300 dark:bg-green-900/50 dark:border-green-700/50',
    5: 'bg-blue-100 border-blue-300 dark:bg-blue-900/50 dark:border-blue-700/50',
  };
  return (
    <div className={`rounded-lg border p-3 ${colors[phase]}`}>
      <div className="text-xs font-mono text-muted-foreground">Phase {phase}</div>
...
|-------|--------------|------------|---------|
| `useCircuitPreviewStore` | List of compiled `Circuit` objects from the last successful compilation | The compile effect (fed by `useCircuitCompiler`) | Circuit selector UI |
...
| `useCircuitLibraryStore` | The `CircuitLibrary` that resolves component names to definitions | The compile effect (fed by `useCircuitCompiler`) | Canvas, ChatPanel, Verilog export |
...
export function FlowDiagram() {
  return (
    <div className="my-6 flex flex-col items-center gap-2 text-sm font-mono">
      ...
      <div className="text-muted-foreground">↕ MCP tools (stdio)</div>
      <div className="rounded-lg border border-blue-300 bg-blue-100 px-4 py-2 text-blue-700 dark:border-blue-700/50 dark:bg-blue-900/30 dark:text-blue-300">
        MCP Server (localhost)
      </div>
      <div className="text-muted-foreground">↓ WebSocket push</div>
      <div className="rounded-lg border border-border bg-muted px-4 py-2 text-foreground">
        Browser App
      </div>
    </div>
