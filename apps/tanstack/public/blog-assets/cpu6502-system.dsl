...
//
// Current ROM: simple.c - writes "Hi!\n" to console
...
// 16KB ROM with reset vector and program (excludes I/O region at $F0xx)
circuit ROM16K {
  input addr_lo: Bus[8]
  input addr_hi: Bus[8]
...
// Includes Console I/O device at $F000
circuit MemoryBus {
  input addr_lo: Bus[8]
  input addr_hi: Bus[8]
  input data_in: Bus[8]
  ...
  impl {
    ...
    node any_ram_rom: Or
    connect ram.responds -> any_ram_rom.a
    connect rom.responds -> any_ram_rom.b

    node any_device: Or
    connect any_ram_rom.out -> any_device.a
    connect console.responds -> any_device.b
...
// - Bus interface (data_in input, addr_lo/addr_hi/data_out/rw outputs)
...
circuit CPU6502Core {
  ...
  impl {
    ...
    // ALSO: During addr_hi fetch (exec_sub1 when needs_2nd_byte), we need PC
    // on the address bus to read the second byte of the absolute address.
    node use_pc_for_addr_temp: Or
    connect is_fetch_state.eq -> use_pc_for_addr_temp.a
    connect control.operand_load -> use_pc_for_addr_temp.b

    node use_pc_for_addr: Or
    connect use_pc_for_addr_temp.out -> use_pc_for_addr.a
    connect control.addr_hi_load -> use_pc_for_addr.b
