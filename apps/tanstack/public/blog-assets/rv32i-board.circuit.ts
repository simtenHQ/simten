...
const RV32I_Core = component('RV32I_Core')
  .in('instruction', bus(32))
  .in('data_read', bus(32))
  .in('net_rx_data', bus(32))
  .in('net_rx_valid', bit)
  .in('net_rx_frame', bit)
  .out('instr_addr', bus(32))
  .out('data_addr', bus(32))
  .out('data_write', bus(32))
  .out('data_mem_read', bit)
  .out('data_mem_write', bit)
  .out('data_funct3', bus(3))
...
const RV32I_Board = component('RV32I_Board')
  .node('cpu', RV32I_Core)
  ...
  .connect(({ in: inp, out, cpu, rom, mem_bus, dmem, uart, imem_load_align, zero32, zero1, pc_display }) => [
