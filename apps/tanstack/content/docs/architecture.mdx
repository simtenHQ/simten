...
@turing-incomplete/ui         ← visual: nodes, canvas, simulation hooks, editor
       ↑
@turing-incomplete/embed      ← convenience: ComponentEmbed, useCircuitSimulator
```
...
| `@turing-incomplete/embed` | Batteries-included wrapper: `ComponentEmbed` (pass TS code, get interactive circuit), `useCircuitSimulator` hook. Depends on core + ui. |
...
The editor has its own compiler (TypeScript editor with Monaco), so it passes the compiled circuit directly to `useCircuitSession`. Embeds don't have their own compiler, so they use `useCircuitSimulator` which compiles internally via `executeComponentCode()`.
...
| `@turing-incomplete/embed` | `ComponentEmbed`, `useCircuitSimulator` — everything needed to embed a circuit |
