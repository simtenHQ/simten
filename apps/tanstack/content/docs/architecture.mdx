...
## From DSL to Simulation

When you write a circuit, it goes through six stages before anything appears on screen:
...
<div className="my-8">
<PipelineStage number={1} title="Parse" subtitle="DSL text → AST">
  A [Chevrotain](https://github.com/Chevrotain/chevrotain) lexer tokenizes the source, the parser produces a concrete syntax tree (CST), and a visitor converts it to an abstract syntax tree (AST).
</PipelineStage>
...
```
@turing-incomplete/core       ← headless: parser, compiler, simulator, session
       ↑
...
| Package | Description |
|---------|-------------|
| `@turing-incomplete/core` | DSL parser, compiler, validator, simulator engine, `SimulationSession`, environmental state, Verilog export. No UI dependencies. |
...
| `@turing-incomplete/embed` | Batteries-included wrapper: `CircuitEmbed` (pass DSL, get interactive circuit), `useCircuitSimulator` hook, `useCompileDSL` hook. Depends on core + ui. |
| `@turing-incomplete/mcp` | MCP server for Claude Code integration |
| `@turing-incomplete/challenges` | Challenge definitions (ALU, Snake, CPU) |
...
| `useCompileDSL(dslCode)` | DSL text | Compilation only | Internal to useCircuitSimulator |
| `useCircuitSimulator(dslCode)` | DSL text | Compile + simulate + UI sugar | Embeds, blog demos, landing page |

The editor has its own compiler (the DSL editor with Monaco), so it passes the compiled circuit directly to `useCircuitSession`. Embeds don't have their own compiler, so they use `useCircuitSimulator` which compiles internally.

Data flows one direction — from the compiler output to the simulation:

```
DSL Editor compiles → { circuits, library }
                           ↓
...
| `@turing-incomplete/embed` | `CircuitEmbed`, `useCircuitSimulator`, `useCompileDSL` — everything needed to embed a circuit |
...
|-----------|-----------|----------|
| Claude → Browser | WebSocket push | DSL updates, waveforms, test results, chat messages |
| Browser → Claude | Channel notifications | User prompts with challenge/circuit context |
| Claude reads state | Pull request/response | Current port values, cycle count, user's DSL |
