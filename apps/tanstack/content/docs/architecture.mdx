...
| `@turing-incomplete/ui` | React components — canvas, embeds, editor stores |
...
| Import | Contents |
|--------|----------|
| `@turing-incomplete/ui/embed` | `CircuitEmbed`, `useCircuitSimulator` — high-level embedding |
| `@turing-incomplete/ui/shared` | `CircuitCanvas`, `WaveformViewer`, `TestResultsPanel` — core rendering |
| `@turing-incomplete/ui/editor` | Full editor kit — stores, components, simulation controller |

The editor uses the same `CircuitCanvas` as blog embeds. Layout is handled by ELK auto-layout with optional `nodePositions` overrides. The DSL is the single source of truth — the canvas is a viewer, not an editor.
