...
export function formatHarnessSuggestion(code: string): string | null {
  ...
  // No interface ports means self-contained circuit
  if (inputs.length === 0 && outputs.length === 0) {
    return null;
  }

  // Check if already has interactive components (Switch, Input, Led, Display etc.)
  const nodeRefs = circuit.nodes.map(n => n.componentRef.toLowerCase());
  const hasInteractiveInputs = nodeRefs.some(r => r === 'switch' || r === 'button' || r === 'input');
  const hasDisplayOutputs = nodeRefs.some(r => r === 'led' || r === 'display' || r === 'hexdisplay' || r === 'screen');
  ...
  lines.push('');
  lines.push(`The circuit "${circuit.name}" has interface ports but no interactive controls.`);
  lines.push('');
  lines.push('**Interface:**');

  if (inputs.length > 0) {
    const inputStr = inputs.map(p => `${p.name}: ${p.portType}`).join(', ');
    lines.push(`- Inputs: ${inputStr}`);
  }

  if (outputs.length > 0) {
    const outputStr = outputs.map(p => `${p.name}: ${p.portType}`).join(', ');
    lines.push(`- Outputs: ${outputStr}`);
  }

  lines.push('');
  lines.push('**Suggestion:** Add Switch/Input controls for inputs and LED/Display for outputs to make the circuit interactive.');
