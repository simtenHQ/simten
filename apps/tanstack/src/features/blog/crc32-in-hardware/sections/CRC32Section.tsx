    ...
    <div className="rounded-xl border border-gray-700/50 bg-gray-100 dark:bg-gray-900/80 overflow-hidden">
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="text-xs text-gray-500 dark:text-gray-500 border-t border-gray-200 dark:border-gray-700/50 pt-3">
          <span className="font-medium text-gray-600 dark:text-gray-400">Standard test vector: </span>
          Feed bytes 49&ndash;57 (ASCII &ldquo;1&rdquo;&ndash;&ldquo;9&rdquo;) one at a time, then XOR the final
          32-bit state with 0xFFFFFFFF. The result should be 0xCBF43926.
        </div>
      </div>
    </div>
  );
    ...
    <section className="py-12">
      ...
      <div className="prose-invert space-y-6 mb-8">
        <p className="text-gray-600 dark:text-gray-300 leading-relaxed">
          The standard CRC-32 algorithm processes one byte per clock cycle. Each byte
          is XOR&rsquo;d into the low 8 bits of the running CRC register, then shifted
          through 8 rounds of the polynomial feedback. The result becomes the new
          register state for the next byte.
        </p>
        <p className="text-gray-600 dark:text-gray-300 leading-relaxed">
          The circuit below shows one byte-step of CRC-32 with a register accumulating
          the state. Change the data byte and click <strong>Step</strong> to process
          it. The register&rsquo;s initial value is 0xFF (truncated from the standard
          0xFFFFFFFF init for the 8-bit demo). After all bytes of a message are
          processed, the final CRC state is XOR&rsquo;d with 0xFFFFFFFF to produce the
          checksum you&rsquo;d find in the Ethernet FCS field.
        </p>
      </div>
      ...
      <div className="mt-8 rounded-lg border border-gray-700/50 bg-gray-100/50 dark:bg-gray-900/50 p-5">
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm leading-relaxed">
          Processing one bit at a time would mean 8 cycles per byte, which is too slow
          for multi-gigabit links. Modern NICs unroll the LFSR computation: instead of
          shifting 8 times sequentially, the hardware precomputes the effect of all 8
          polynomial shifts in parallel and produces the result in a single clock
          cycle. At 100 Gbps, a NIC must compute CRC-32 over roughly 12.5 billion
          bytes per second. The unrolled hardware does exactly that with a fixed amount
          of combinational logic &mdash; no loops, no branches.
        </p>
      </div>
    </section>
  );
