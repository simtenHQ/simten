...
import {
  Input, HexDisplay, Constant, Led, Switch,
  RightShifter, SignedAdder, BusNot, SignedComparator,
  Mux, Register, Incrementer, Comparator, BitSlice,
...
export const RightShiftDemo = circuit('RightShiftDemo', {
  ...
  connect: ({ value, shift, shifter, result }) => [
...
export const RotationStep = circuit('RotationStep', {
  ...
  connect: ({ x, y, shift, one, zero, yShifted, yNeg, xMinusY, xPlusY, displaySub, displayAdd }) => [
...
export const SignDetection = circuit('SignDetection', {
  ...
  connect: ({ angle, zero, cmp, positiveLed, addVal, subVal, result, display }) => [
    angle.out.to(cmp.a),
    zero.out.to(cmp.b),
...
export const IterationControl = circuit('IterationControl', {
  ...
  connect: ({ iter, eight, inc, shouldContinue, display, doneLed }) => [
...
export const AngleLookup = circuit('AngleLookup', {
  ...
  connect: ({ iteration, angle0, angle1, angle2, angle3, angle4, angle5, angle6, angle7, bit0, bit1, bit2, mux01, mux23, mux45, mux67, mux0123, mux4567, angleSel, display }) => [
...
/** @deprecated Use CORDICCircuit instead */
